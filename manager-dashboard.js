document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add event listener for Projects sidebar link
    const projectsLink = document.querySelector('.sidebar-nav li:nth-child(2) a');
    projectsLink.addEventListener('click', function(e) {
        e.preventDefault();
        window.location.href = 'projects.html';
    });
    
    // Add event listener for Team Members sidebar link
    const teamMembersLink = document.querySelector('.sidebar-nav li:nth-child(3) a');
    teamMembersLink.addEventListener('click', function(e) {
        e.preventDefault();
...
function updateDashboardStats(projects) {
    ...
    const inProgressCount = projects.filter(p => p.status === 'ongoing').length;
    document.getElementById('in-progress-projects').textContent = inProgressCount;
    ...
    projects.forEach(project => {
        if (project.teamMembers && Array.isArray(project.teamMembers)) {
            project.teamMembers.forEach(member => teamMemberSet.add(member));
        }
    });
    
    document.getElementById('team-members').textContent = teamMemberSet.size || 0;
}
